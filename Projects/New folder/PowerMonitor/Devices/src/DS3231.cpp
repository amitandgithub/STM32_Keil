...
char* DS3231::getTimeStr(char* pBuffer)
{
    ...
    //intToStr(t.date, &pBuffer[0], 2);
	  std::memcpy(&pBuffer[0], (std::to_string(t.date)).c_str(),2);
    pBuffer[2] = '-';
    //intToStr(t.mon, &pBuffer[3], 2);
	  std::memcpy(&pBuffer[3], (std::to_string(t.mon)).c_str(),2);
    pBuffer[5] = '-';
    //intToStr(t.year, &pBuffer[6], 4);
	  std::memcpy(&pBuffer[6], (std::to_string(t.year)).c_str(),4);
    // Time (startsw with 10)
    //intToStr(t.hour, &pBuffer[10], 2);
	  std::memcpy(&pBuffer[10], (std::to_string(t.year)).c_str(),2);
    pBuffer[12] = ':';
    //intToStr(t.min, &pBuffer[13], 2);
	  std::memcpy(&pBuffer[13], (std::to_string(t.min)).c_str(),2);
    pBuffer[15] = ':';
    //intToStr(t.sec, &pBuffer[16], 2);
	  std::memcpy(&pBuffer[16], (std::to_string(t.sec)).c_str(),2);
    return pBuffer;
